...
let expand_deps_from_type pkg deptype = match deptype with
  | Depends      -> pkg.depends
...
let dependsAUR pkgnames table =
  ...
  let rec resolve depth batch =
    if depth >= aur_callback_max || batch = [] then
      ...
      let all_deps pkg = List.concat_map (fun deptype ->
        List.map (fun dep -> (dep, deptype)) (expand_deps_from_type pkg deptype)
        ) all_dep_types
      in

      (*collecting next and filling results tally and pkgdeps and pkgmap at same time *)
      let next = List.concat_map (fun pkg ->

        List.iter (fun prov_spec ->
           match String.split_on_char '=' prov_spec with
          | [prov; prov_version] -> Hashtbl.add pkgmap prov (pkg.name, prov_version)
          | [prov] -> Hashtl.add pkgmap prov (pkg.name, None)
          | _ -> ()
        ) pkg.provides;
