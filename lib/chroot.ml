...
let get_default_paths_exn machine ?suffix ?directory ?pacman_conf ?makepkg_conf () =
  ...
  let resolve kind opt paths =
    ...
    | Some p ->
        raise (UsageError (Printf.sprintf "chroot: %s configuration not found: %s" kind p))
...
let bindmounts_rw_from_conf pacman_conf =
  ...
  let inp =
    Unix.open_process_args_in "pacman-conf" [| "pacman-conf"; "--config"; pacman_conf |]
  in
  In_channel.input_lines inp
  |> List.filter_map (fun line ->
      if Str.string_match r line 0 then Some (Str.matched_group 1 line) else None)

let chroot ~build ~update ~create ~path ?directory ?(bind_ro = []) ?(bind_rw = [])
    ?(pkgnames = []) ?(makechrootpkg_args = [ "-cu" ]) ?(makechrootpkg_makepkg_args = [])
    ?(env = Unix.environment ()) ?suffix ?pacman_conf ?makepkg_conf () =
  ...
  if not (update || build || create || path) then
    ...
    if create then begin
      ...
      if update then begin
        ...
        let bind_rw = List.map (fun b -> "--bind=" ^ b) (bind_rw @ pacman_conf_bindmounts_rw) in
        run_exn "sudo"
      ...
      if build then begin
        ...
        @ bind_ro @ bind_rw @ makechrootpkg_args @ [ "--" ] @ makechrootpkg_makepkg_args
      end;
      if path then
