...
let chroot ?suffix ?pacman_conf ?makepkg_conf ~build ~update ~create ~path
    ~directory:opt_directory ~bind_ro:opt_bind_ro ~bind_rw:opt_bind_rw ~pkgnames
  ...
  let makepkg_conf =
    resolve_conf "makepkg" makepkg_conf default_makepkg_paths
  in
    ...
    if create then begin
      ...
      let base_packages =
        if pkgnames <> [] then pkgnames
          ...
          run_read_all
            ("pacini", [ "pacini"; "--section=multilib"; pacman_conf ])
          <> ""
          && machine = "x86_64"
      ...
      if update then
        ...
        let realpath =
          String.trim
            (run_read_all
               ("realpath", [ "realpath"; "--"; directory // "root" ]))
        in
